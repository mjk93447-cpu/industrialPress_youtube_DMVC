...
def roadmap_items(repo_root: Path) -> list[RoadmapItem]:
    ...
    return [
        ...
        RoadmapItem(
            "phase1",
            "metadata_crawl",
            "YouTube metadata crawl script with candidate normalization",
            ("scripts/youtube_metadata_crawl.py",),
            optional=False,
        ),
        RoadmapItem(
            "phase1",
            "rights_gate",
            "Rights transition and training eligibility hard-fail",
            ("src/dmvc_poc/data/rights.py",),
            optional=False,
        ),
        RoadmapItem(
            "phase1",
        ...
        RoadmapItem(
            "phase3",
            "evaluate_cli",
            "Evaluation script exporting metrics",
            ("scripts/evaluate.py",),
            optional=False,
        ),
        RoadmapItem(
